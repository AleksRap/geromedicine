...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var coversArticles = document.querySelectorAll('.article__pic');
  if (coversArticles.length) coversArticles.forEach(function (cover) {
    return cover.style.height = "".concat(cover.clientWidth / 3.6, "px");
  });
  /**
   * Обложки статей в соотношении 3.6
   */

  var coversCardsArt = document.querySelectorAll('.article-card__img');
  if (coversCardsArt.length) coversCardsArt.forEach(function (cover) {
    return cover.style.height = "".concat(cover.clientWidth / 3.6, "px");
  });
  /**
  ...
  screenWidth <= 480 ? bannersStock.forEach(function (banner) {
    return banner.style.height = "".concat(banner.clientWidth / 1.65, "px");
  ...
  if (screenWidth < 1200) {
    ...
    navigation.addEventListener('click', function (event) {
      ...
      var subItem = event.target.closest('.navigation__link');
      if (!subItem) return;
